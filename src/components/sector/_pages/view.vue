  ...
  <v-container fluid>
    <v-card>
      <v-toolbar color="toolbarColor" dense dark>
        <v-toolbar-title>sector Details</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-btn text to="/categories"><v-icon left>mdi-arrow-left</v-icon>Back</v-btn>
      </v-toolbar>
      <v-card-text>
        {{ dataItem.url }}
...
export default {
  ...
  methods: {
    ...
    save(data) {
      const url = `/sector/edit/${this.dataItem._id}`;
      const self = this;
      this.$store
