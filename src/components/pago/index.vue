        ...
        <v-row>
          <v-col cols="12">
            <v-btn text color="accent" @click="openModal">
              <v-icon left>mdi-plus</v-icon>New Page
            </v-btn>
            <v-data-table :headers="headers" :items="pago">
               <!-- <template v-slot:[`item.name`]="{ item }">
            <v-btn color="accent" text :to="`/sections?url=${item.url}`">
              {{item.name}}
            </v-btn>
            </template> -->
            <template v-slot:[`item.url`]="{ item }">
             <v-btn color="accent" text v-if="item.url === 'home'" to="/sector">{{item.url}}</v-btn>
               <v-btn color="accent" text v-if="item.url === 'about'" to="/sectorb">{{item.url}}</v-btn>
            </template>
            </v-data-table>
          </v-col>
        </v-row>
